...
$(function(){if(typeof FastClick!=="undefined"){
// Don't attach to body if undefined
if(typeof document.body!=="undefined"){FastClick.attach(document.body)}}});
// private Fast Selector wrapper,
// returns jQuery object. Only use where
// getElementById is not available.
var S=function(selector,context){if(typeof selector==="string"){if(context){var cont;if(context.jquery){cont=context[0];if(!cont){return context}}else{cont=context}return $(cont.querySelectorAll(selector))}return $(document.querySelectorAll(selector))}return $(selector,context)};
// Namespace functions.
var attr_name=function(init){var arr=[];if(!init){arr.push("data")}if(this.namespace.length>0){arr.push(this.namespace)}arr.push(this.name);return arr.join("-")};var add_namespace=function(str){var parts=str.split("-"),i=parts.length,arr=[];while(i--){if(i!==0){arr.push(parts[i])}else{if(this.namespace.length>0){arr.push(this.namespace,parts[i])}else{arr.push(parts[i])}}}return arr.reverse().join("-")};
...
// use rAF
window.requestAnimationFrame=requestAnimationFrame;window.cancelAnimationFrame=cancelAnimationFrame;if(jqueryFxAvailable){jQuery.fx.timer=function(timer){if(timer()&&jQuery.timers.push(timer)&&!animating){animating=true;raf()}};jQuery.fx.stop=function(){animating=false}}}else{
// polyfill
...
random_str:function(){if(!this.fidx){this.fidx=0}this.prefix=this.prefix||[this.name||"F",(+new Date).toString(36)].join("-");return this.prefix+(this.fidx++).toString(36)},
// Description:
...
modal.on("open.fndtn.reveal",function(e){if(e.namespace!=="fndtn.reveal")return});modal.on("open.fndtn.reveal").trigger("open.fndtn.reveal");if(open_modal.length<1){this.toggle_bg(modal,true)}if(typeof ajax_settings==="string"){ajax_settings={url:ajax_settings}}var openModal=function(){if(open_modal.length>0){if(settings.multiple_opened){self.to_back(open_modal)}else{self.hide(open_modal,settings.css.close)}}
...
modal.trigger("close.fndtn.reveal");if(settings.multiple_opened&&open_modals.length===1||!settings.multiple_opened||modal.length>1){self.toggle_bg(modal,false);self.to_front(modal)}if(settings.multiple_opened){var isCurrent=modal.is(":not(.toback)");self.hide(modal,settings.css.close,settings);if(isCurrent){
// remove the last modal since it is now closed
openModals.pop()}else{
// if this isn't the current modal, then find it in the array and remove it
openModals=$.grep(openModals,function(elt){var isThis=elt[0]===modal[0];if(isThis){
// since it's not currently in the front, put it in the front now that it is hidden
// so that if it's re-opened, it won't be .toback
self.to_front(modal)}return!isThis})}
// finally, show the next modal in the stack, if there is one
if(openModals.length>0){self.to_front(openModals[openModals.length-1])}}else{self.hide(open_modals,settings.css.close,settings)}}},close_targets:function(){var base="."+this.settings.dismiss_modal_class;if(this.settings.close_on_background_click){return base+", ."+this.settings.bg_class}return base},toggle_bg:function(modal,state){if(this.S("."+this.settings.bg_class).length===0){this.settings.bg=$("<div />",{"class":this.settings.bg_class}).appendTo("body").hide()}var visible=this.settings.bg.filter(":visible").length>0;if(state!=visible){if(state==undefined?visible:!state){this.hide(this.settings.bg)}else{this.show(this.settings.bg)}}},show:function(el,css){
// is modal
...
if(getAnimationData(settings.animation).fade){return el.fadeIn(settings.animation_speed/2)}this.locked=false;return el.show()},to_back:function(el){el.addClass("toback")},to_front:function(el){el.removeClass("toback")},hide:function(el,css){
// is modal
